...
# import pymysql
# # 连接database
# conn = pymysql.connect(host="192.168.43.5", user="root", password="", database="pen", charset="utf8")
...
class Ui_MainWindow(QtWidgets.QWidget):
    ...
    def set_ui(self):
        ...
        self.addface.setMinimumHeight(30)
        ...
        self.showcamera.setFixedSize(1920, 1080)
        ...
        self.setWindowTitle(u'多目标人脸门禁')
        self.setStyleSheet("C:/Users/26440/Pictures/SavedPictures/timg.jfif")
                ...
                for face in faces:
                    ...
                    if face.name is not None:
                        if face.name == 'unknown':
                            draw.text((face_bb[0], face_bb[1]), '陌生人', font=font, fill=fillColor1)
                        else:
                            # TODO:传入数据库
                            draw.text((face_bb[0], face_bb[1]), face.name, font=font, fill=fillColor2)
                            # cursor = conn.cursor()
                            # sql = "INSERT INTO face(name,time) VALUES ( %s,%s);"
                            # name = face.name
                            # time = tm.strftime('%Y-%m-%d %H:%M:%S',tm.localtime(tm.time()))
                            # # 执行SQL语句
...
if __name__ == '__main__':
    ...
    #sys.exit(app.exec_())
    # cursor = conn.cursor()
    # sql = "INSERT INTO renlian(name) VALUES ( %s);"
    # name = "Alex"
    # # 执行SQL语句
